...
public class BrowseHire extends JPanel implements ActionListener {
	...
	public void tableInitialize() {
		try {
			DefaultTableModel tableModel = (DefaultTableModel) TableStus
					.getModel();
			// 定义一维数组作为表格标题
			Object[] tableHead = { "资产代码",  "企业代码","租赁费用 ","出租日期","经办人" };
			// 定义二维数组作为表格数据
			JDBCFile db = new JDBCFile();
			
			String sqlstr = "select *  from 出租表";
			int row=0;
			...
			while (rs.next()) {
				...
				tableData[i][1] = rs.getString("企业代码");
				tableData[i][2] = rs.getString("租赁费用");
				tableData[i][3] = rs.getString("出租日期");
