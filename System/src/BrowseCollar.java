...
public class BrowseCollar extends JPanel implements ActionListener {
	...
	public void tableInitialize() {
		try {
			DefaultTableModel tableModel = (DefaultTableModel) TableStus
					.getModel();
			// 定义一维数组作为表格标题
			Object[] tableHead = { "资产代码",  "部门编号", "领用日期","经办人" };
			// 定义二维数组作为表格数据
			JDBCFile db = new JDBCFile();
			
			String sqlstr = "select 资产代码,部门编号,领用日期,经办人  from 领用表";
			int row=0;
			...
			Object[][] tableData = new Object[row][4];
			...
			while (rs.next()) {
				...
				tableData[i][1] = rs.getString("部门编号");
				tableData[i][2] = rs.getString("领用日期");
				tableData[i][3] = rs.getString("经办人");
