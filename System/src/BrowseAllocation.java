...
public class BrowseAllocation extends JPanel implements ActionListener {

	JTable TableStus;

	public BrowseAllocation() {
	...
	public void tableInitialize() {
		try {
			DefaultTableModel tableModel = (DefaultTableModel) TableStus
					.getModel();
			// 定义一维数组作为表格标题
			Object[] tableHead = { "资产代码",  "调入部门编号","调出部门编号 ","调拨日期","经办人" };
			// 定义二维数组作为表格数据
			JDBCFile db = new JDBCFile();
			
			String sqlstr = "select *  from 调拨表";
			int row=0;
			...
			while (rs.next()) {
				...
				tableData[i][1] = rs.getString("调入部门编号");
				tableData[i][2] = rs.getString("调出部门编号");
				tableData[i][3] = rs.getString("调拨日期");
